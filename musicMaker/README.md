...
The generated module preserves `r0` and `r1`; its sample loader also preserves `r2`. It reserves an eight-byte slot in the `0x000100 - 0x0001FF` system scratch area for its step, delay, playback, and loop states. The slot is derived from the export filename so separately named generated songs do not normally share runtime state. Change the prefixed state-base constant near the top of an exported file if its slot collides with another module or game data.
